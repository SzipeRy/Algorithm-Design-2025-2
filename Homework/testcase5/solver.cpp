...
// Dynamic memory pointers to handle any N safely
int* tile_epoch_arr;
int* tile_dist;
int* tile_parent;
int* blank_epoch_arr;
...
// O(L) Stack-Based String Optimizer (Instantly cleans up redundant wiggles)
...
string simulate_order(const vector<int>& order) {
    ...
    string ans = "";
    ans.reserve(200000); // Pre-allocate memory to prevent slow string expansions
    ...
    for (int dest_idx : order) {
        ...
        while(!q.empty()) {
            ...
            if (tile_dist[u] > min_dist) continue; // Prune deeper searches
        ...
        // 2. Generate a randomized path for the tile to its destination
        current_epoch++;
        q.push(chosen_cand);
...
// Generates uniquely randomized topological mapping strategies to continuously compress constraints
...
int main() {
    ...
    while (chrono::duration<double>(chrono::high_resolution_clock::now() - start_time).count() < 1200.0) {
